...
do
	...
	then
		...
		then
			texte="$ligne"
			texte+='"'$PATH_INSTALL'"'
			echo "$texte" >> "enlignecpp.sh"
		else
			echo "$ligne" >> "enlignecpp.sh"
		fi
	fi
	...
	if [[ $okcpp = 0 && $ligne = "#=== CODE SOURCE cpp ===" ]]
	then
		okcpp=1
	fi
done <install.sh
...
Temp+="/temp.cpp"
copy_Temp=$PATH_CENLIGNE
copy_Temp+="/copy_temp.cpp"
echo "#include <iostream>" >$Temp
echo "using namespace std;" >>$Temp
echo "int main(int argc, char const *argv[]){" >>$Temp
echo "}" >>$Temp
echo "---------- Environnement en ligne C++ ----------"
opt=()
...
do
	...
	then
		...
		if [ "$path_to_file" = "nul part" ]
		then
			path_to_file="$PATH_CENLIGNE/savep"
		else
			path_to_file+="/"
			read -r -p "Nom de la sauvegarde : " name_of_file
			path_to_file+="$name_of_file"
	...
	then		
		...
		read -r prompt lib <<< "$commande"
		$COMPIL $copy_Temp -o "$PATH_CENLIGNE/b.out" "${opt[@]}" "$lib"
